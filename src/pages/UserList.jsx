...
const UserList = () => {
  ...
  const [editForm, setEditForm]     = useState({ allowedAttempts: "3", timeoutDays: "5" });
  ...
  const getUsers = async () => {
    ...
    const enriched = await Promise.all(
      userList.map(async (user) => {
        const assigned = await apiGet('/exam/getallexam-by-partyId/' + user.partyId);
                ...
                return (
                  <UserCard key={user.partyId} $accent={pal.accent} $delay={`${index * 0.03}s`} >
                    ...
                    <UserCardTop>
                      ...
                      <UserCardActions>
                         <TooltipWrapper>
                          <TooltipChip>View user</TooltipChip>
                          <ActionIconBtn
                            className="edit"
                            onClick={()=>{navigate(`/userpage/${user.partyId}`)}}
                          >
